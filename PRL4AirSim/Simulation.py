...
class Sim(object):
    def __init__(self, image_shape, num_drones):
        ...
        self.areans_train_long = np.array(
            [
                ...
                [
                    Utils.convert_pos_UE_to_AS(
                        self.origin_UE, np.array([9030.0, -6760.0, 1000.0])
                    ),
                    Utils.convert_pos_UE_to_AS(
                        self.origin_UE, np.array([14060.0, -6760.0, 1000.0])
                    ),
                ]
            ]
        )
    ...
    def gatherAllObservations(self):
        ...
        if useNewMethod:
            ...
            responses = [
                airsim.ImageResponse.from_msgpack(response_raw[0])
                for response_raw in responses_raw
            ]
            imageDepths = [
                airsim.list_to_2d_float_array(
    ...
    def doActionBatch(self):
        ...
        for droneObject in self.droneObjects:
            ...
            vy_vec.append(
                quad_vel.y_val + y_val_offset if droneObject.reseting == False else 0
            )
    ...
    def resetBatch(self):
        ...
        poses = [
            airsim.Pose(
                airsim.Vector3r(
                    self.areans[droneObject.currentArena][0][0],
                    self.areans[droneObject.currentArena][0][1]
                    + self.randomPoseInArena(),
        ...
        for droneObject in self.droneObjects:
            ...
            currentPos_x_AS = (
                Utils.getClient()
                .getMultirotorState(droneObject.droneName)
                .kinematics_estimated.position.to_numpy_array()[0]
            )
            droneObject.distanceFromGoal = abs(
                currentPos_x_AS - self.areans[droneObject.currentArena][1][0]
            )
    ...
    def resetStep(self, droneObject: DroneObj):
        if droneObject.reseting == True:
            ...
            if (
                ...
                currentPos_x_AS = state.kinematics_estimated.position.to_numpy_array()[
                    0
                ]
                droneObject.distanceFromGoal = abs(
                    currentPos_x_AS - self.areans[droneObject.currentArena][1][0]
