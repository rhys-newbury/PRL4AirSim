...
def connectClient(trainer_ip_address, ue_ip_address, trainer_port=29000, ue_port=41451):
    ...
    try:
        ...
        print(
            "Cannot Connect to Multirotor Client, please ensure Unreal Engine is running with AirSim plugin"
        )
    ...
    try:
        model_server = msgpackrpc.Client(
            msgpackrpc.Address(trainer_ip_address, trainer_port)
        )
...
def fixed_simGetImages(requests, vehicle_name="", external: bool = False):
    responses_raw = getClient().client.call(
...
def handleImage(
    droneName: str, cameraName: str, imageType: airsim.ImageType
) -> np.array:
    if imageType == airsim.ImageType.Scene:
        ...
        imageRGB = image1d.reshape(
            (imageResponses[0].height, imageResponses[0].width, 3)
        )
        return imageRGB
    elif (
        imageType == airsim.ImageType.DepthPlanar
        or imageType == airsim.ImageType.DepthVis
        or imageType == airsim.ImageType.DepthPerspective
    ):
        imageResponses = fixed_simGetImages(
            [airsim.ImageRequest(cameraName, airsim.ImageType.DepthPlanar, True, True)],
            droneName,
            False,
        )
        imageDepth = airsim.list_to_2d_float_array(
            imageResponses[0].image_data_float,
            imageResponses[0].width,
...
def showDepthImage(droneName: str):
    imageResponses = fixed_simGetImages(
        [airsim.ImageRequest("depth_cam", airsim.ImageType.DepthPlanar, True, True)],
        droneName,
        False,
    )
    imageDepth = airsim.list_to_2d_float_array(
