...
def testSampleFromStorageTrainer():
    ...
    for i in range(50):
        storage_server.agent.memory.push(
            {"image": np.zeros(shape=(2, 32, 32)), "velocity": np.zeros(shape=(3,))},
            1,
            {"image": np.zeros(shape=(2, 32, 32)), "velocity": np.zeros(shape=(3,))},
            0.1,
            1,
    ...
    for i in range(len(state)):
        transition = ReplayMemory.Transition(
            Utils.convertStateDicToNumpyDic(state[i]),
            action[i],
...
if __name__ == "__main__":
    ...
    try:
        model_server = msgpackrpc.Client(
            msgpackrpc.Address("127.0.0.1", int(arguments["storage_port"]))
        )
        ...
        print(
            "Ip address = {} and port {}".format(
                "127.0.0.1", int(arguments["storage_port"])
            )
        )
        print(e)
        exit(1)
    ...
    while True:
        state, action, next_state, reward, not_done = model_server.call(
            "sampleFromStorage"
        )
        if state == None:
