...
class Storage(object):
    ...
    def pushMemory(self, state, action, next_state, reward, not_done):
        self.agent.memory.push(
            Utils.convertStateDicToNumpyDic(state),
            action,
            Utils.convertStateDicToNumpyDic(next_state),
            reward,
            not_done,
        )
    ...
    def finishEpisode(self, finalDistance, totalReward):
        ...
        if self.total_episodes % 1000 == 0 and self.total_episodes != 0:
            ...
            artifact = wandb.Artifact(
                "dqn_3D_{}_EP_{}".format(self.run_name, self.total_episodes),
                type="model",
                description="Episode {}".format(self.total_episodes),
            )
            artifact.add_file("{}/ModelSaves/dqn.pth".format(pathlib.Path().resolve()))
            self.run.log_artifact(artifact)

    def setNetworkTrainIteration(self, trainIteration):
        wandb.log({"metric/Train Iteration": trainIteration}, step=self.total_episodes)

    def sampleFromStorage(self):
        if (
            len(self.agent.memory) >= self.agent.replayMemory_size
            or len(self.agent.memory) >= self.agent.batch_size
        ):
...
def testSampleFromStorage():
    ...
    for i in range(50):
        storage_server.agent.memory.push(
            {"image": np.zeros(shape=(32, 32)), "position": np.zeros(shape=(3,))},
            1,
            {"image": np.zeros(shape=(32, 32)), "position": np.zeros(shape=(3,))},
            0.1,
            1,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Storage", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
