...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="PyClient", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    client, model_server = Utils.connectClient(
        trainer_ip_address=trainer_ip_address,
        ue_ip_address=ue_ip_address,
        trainer_port=storage_port,
        ue_port=ue_port,
    )
    times = []
    ...
    simulation = Simulation.Sim(
        image_shape=Utils.getConfig()["state_space"],
        num_drones=Utils.getConfig()["num_drones"],
    )
