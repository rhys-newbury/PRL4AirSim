...
class DQNTrainer(object):
    def __init__(
        ...
        self.network = DQNetwork.DQNetwork(
            learningRate, self.n_actions, image_input_dims
        )
        self.target_network = DQNetwork.DQNetwork(
            learningRate, self.n_actions, image_input_dims
        )
    ...
    def learn(self, transitions):
        ...
        Q_target = (
            rewards
            + self.target_network.forward(*next_state).max(dim=1)[0]
            * self.discount_factor
            * not_done
        )

        loss = self.network.loss(Q_old.double(), Q_target.double()).to(
            self.network.device
        )
        loss.backward()
